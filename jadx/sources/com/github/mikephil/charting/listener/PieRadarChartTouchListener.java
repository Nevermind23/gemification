...
public class PieRadarChartTouchListener extends ChartTouchListener<PieRadarChartBase<?>> {
    ...
    private float calculateVelocity() {
        ...
        float f3 = angularVelocitySample3.angle;
        if (f2 >= f3) {
            z = true;
        }
        if (((double) Math.abs(f2 - f3)) > 270.0d) {
            z = !z;
        }
        float f4 = angularVelocitySample2.angle;
        float f5 = angularVelocitySample.angle;
        if (((double) (f4 - f5)) > 180.0d) {
            angularVelocitySample.angle = (float) (((double) f5) + 360.0d);
        } else if (((double) (f5 - f4)) > 180.0d) {
            angularVelocitySample2.angle = (float) (((double) f4) + 360.0d);
        }
        ...
        if (!this.mGestureDetector.onTouchEvent(motionEvent) && ((PieRadarChartBase) this.mChart).isRotationEnabled()) {
            ...
            if (action == 0) {
                ...
                if (this.mTouchMode == 0) {
                    ...
                    if (ChartTouchListener.distance(x, mPPointF2.f11822x, y, mPPointF2.f11823y) > Utils.convertDpToPixel(8.0f)) {
