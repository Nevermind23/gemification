...
public abstract class Utils {
    ...
    public static void drawImage(Canvas canvas, Drawable drawable, int i, int i2, int i3, int i4) {
        ...
        instance.f11822x = (float) (i - (i3 / 2));
        instance.f11823y = (float) (i2 - (i4 / 2));
        ...
        canvas.translate(instance.f11822x, instance.f11823y);
    ...
    public static void drawMultilineText(Canvas canvas, StaticLayout staticLayout, float f, float f2, TextPaint textPaint, MPPointF mPPointF, float f3) {
        ...
        float f5 = lineCount + FLOAT_EPSILON;
        Paint.Align textAlign = textPaint.getTextAlign();
        textPaint.setTextAlign(Paint.Align.LEFT);
        if (f3 != FLOAT_EPSILON) {
            float f6 = f4 - (width * 0.5f);
            float f7 = f5 - (lineCount * 0.5f);
            if (!(mPPointF.f11822x == 0.5f && mPPointF.f11823y == 0.5f)) {
                FSize sizeOfRotatedRectangleByDegrees = getSizeOfRotatedRectangleByDegrees(width, lineCount, f3);
                f -= sizeOfRotatedRectangleByDegrees.width * (mPPointF.f11822x - 0.5f);
                f2 -= sizeOfRotatedRectangleByDegrees.height * (mPPointF.f11823y - 0.5f);
            ...
            float f8 = mPPointF.f11822x;
            if (!(f8 == FLOAT_EPSILON && mPPointF.f11823y == FLOAT_EPSILON)) {
                f4 -= width * f8;
                f5 -= lineCount * mPPointF.f11823y;
            }
            canvas.save();
    ...
    public static void drawXAxisValue(Canvas canvas, String str, float f, float f2, Paint paint, MPPointF mPPointF, float f3) {
        ...
        if (f3 != FLOAT_EPSILON) {
            ...
            if (!(mPPointF.f11822x == FLOAT_EPSILON && mPPointF.f11823y == FLOAT_EPSILON)) {
                f4 -= ((float) mDrawTextRectBuffer.width()) * mPPointF.f11822x;
                f5 -= fontMetrics * mPPointF.f11823y;
            }
    ...
    public static String formatNumber(float f, int i, boolean z, char c) {
        ...
        long round = (long) Math.round(f2 * ((float) iArr[length]));
        int i4 = 34;
        boolean z4 = false;
        while (true) {
